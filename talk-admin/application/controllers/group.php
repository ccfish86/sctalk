...
class Group extends TT_Controller {
	...
	public function getMember()
	{
		$id = $this->input->post('id');
		$perpage = 10000;
		$users = $this->grouprelation_model->getList(array('status'=>0,'group_id'=>$id), '*', 0, $perpage);
		foreach ($users as $key => $value) {
			$_data = $this->user_model->getOne(array('id'=>$value['user_id']));
	...
	public function changeMember()
	{
		$add = array(
			...
			'user_id_list'  => array(intval($this->input->post('user_id')))
		);                  
