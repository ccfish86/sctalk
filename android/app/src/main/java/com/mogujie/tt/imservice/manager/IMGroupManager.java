...
public class IMGroupManager extends IMManager {
    ...
    public void onRepNormalGroupList(IMGroup.IMNormalGroupListRsp normalGroupListRsp) {
        ...
        for(IMBaseDefine.GroupVersionInfo groupVersionInfo:versionInfoList ){
            long groupId =  groupVersionInfo.getGroupId();
    ...
    public void  reqGroupDetailInfo(long groupId){
    ...
    public void onRepGroupDetailInfo(IMGroup.IMGroupInfoListRsp groupInfoListRsp){
        ...
        long userId = groupInfoListRsp.getUserId();
        long loginId = imLoginManager.getLoginId();
     ...
     */
    public void reqCreateTempGroup(String groupName, Set<Long> memberList){
     ...
     * 可能会触发头像的修改
     */
    public void reqRemoveGroupMember(long groupId,Set<Long> removeMemberlist){
     ...
     * 可能会触发头像的修改
     */
    public void reqAddGroupMember(long groupId,Set<Long> addMemberlist){
        reqChangeGroupMember(groupId,IMBaseDefine.GroupModifyType.GROUP_MODIFY_TYPE_ADD, addMemberlist);
    }

    private void reqChangeGroupMember(long groupId,IMBaseDefine.GroupModifyType groupModifyType, Set<Long> changeMemberlist) {
        logger.i("group#reqChangeGroupMember, changeGroupMemberType = %s", groupModifyType.toString());

        final long loginId = imLoginManager.getLoginId();
    ...
    public void onReqChangeGroupMember(IMGroup.IMGroupChangeMemberRsp groupChangeMemberRsp){
        ...
        long groupId = groupChangeMemberRsp.getGroupId();
        List<Long> changeUserIdList = groupChangeMemberRsp.getChgUserIdListList();
     ...
     * */
    public void reqShieldGroup(final long groupId,final int shieldType){
        final GroupEntity entity =  groupMap.get(groupId);
        if(entity == null){
            logger.i("GroupEntity do not exist!");
            return;
        }
        final long loginId = IMLoginManager.instance().getLoginId();
    ...
    public void receiveGroupChangeMemberNotify(IMGroup.IMGroupChangeMemberNotify notify){
       long groupId =  notify.getGroupId();
       int changeType = ProtoBuf2JavaBean.getGroupChangeType(notify.getChangeType());
       List<Long> changeList =  notify.getChgUserIdListList();

       List<Long> curMemberList = notify.getCurUserIdListList();
       if(groupMap.containsKey(groupId)){
	...
	public List<GroupEntity> getNormalGroupList() {
		...
		for (Entry<Long, GroupEntity> entry : groupMap.entrySet()) {
    ...
    public List<GroupEntity>  getSearchAllGroupList(String key){
        ...
        for(Map.Entry<Long,GroupEntity> entry:groupMap.entrySet()){
	...
	public List<UserEntity> getGroupMembers(int groupId) {
        ...
        Set<Long> userList = group.getlistGroupMemberIds();
    ...
    /**------set/get 的定义*/
    public Map<Long, GroupEntity> getGroupMap() {
        return groupMap;
    }
