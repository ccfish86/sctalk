...
public class IMMessageManager extends IMManager{
    ...
    public List<MessageEntity> loadHistoryMsg(MessageEntity entity,int pullTimes){
        ...
        long reqLastMsgId = entity.getMsgId() - 1;
        long loginId = IMLoginManager.instance().getLoginId();
     ...
     * @return
     */
    private List<MessageEntity> doLoadHistoryMsg(int pullTimes,final long peerId,final int peerType, final String sessionKey,long lastMsgId,int lastCreateTime,int count){
        if(lastMsgId <1 || TextUtils.isEmpty(sessionKey)){
            return Collections.emptyList();
        }
        if(count > lastMsgId){
    ...
    private void doRefreshLocalMsg(RefreshHistoryMsgEvent hisEvent){
        /**check DB数据的一致性*/
        long lastSuccessMsgId = hisEvent.lastMsgId;
     ...
     * 所以要保证DB数据没有问题
     */
    public void refreshDBMsg(long peerId,int peedType,String chatKey,long lastMsgId,int refreshCnt){
        if(lastMsgId <1){return;}
        long beginMsgId = lastMsgId - refreshCnt;
        if(beginMsgId<1){beginMsgId=1;}

        // 返回的结果是升序
        List<Long> msgIdList =  dbInterface.refreshHistoryMsgId(chatKey, beginMsgId, lastMsgId);
        if(msgIdList.size() == (lastMsgId-beginMsgId+1)){
            logger.d("refreshDBMsg#do need refresh Message!,cause sizeOfList is right");
            return;
        }
        // 查找缺失的msgid
        List<Long> needReqList = new ArrayList<>();
        for(long startIndex=beginMsgId,endIndex=lastMsgId;startIndex<=endIndex;startIndex++){
    ...
    private void reqMsgById(long peerId,int sessionType,List<Long> msgIds){
     ...
     * network 请求历史消息
     */
    public  void reqHistoryMsgNet(long peerId,int peerType, long lastMsgId, int cnt){
        long loginId = IMLoginManager.instance().getLoginId();
     ...
     * */
     public void onReqHistoryMsg(IMMessage.IMGetMsgListRsp rsp) {
         ...
         long peerId = rsp.getSessionId();
         String sessionKey = EntityChangeEngine.getSessionKey(peerId, sessionType);
         long msgBegin = rsp.getMsgIdBegin();
