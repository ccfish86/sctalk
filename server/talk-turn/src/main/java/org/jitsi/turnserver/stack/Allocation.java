...
public class Allocation {
     ...
     * Our class logger.
     */
    private static final Logger logger = LoggerFactory.getLogger(Allocation.class.getName());
    ...
    /**
     * The <tt>Thread</tt> which expires the <tt>Permission</tt>s of this <tt>Allocation</tt> and
     * removes them from {@link #permissions}.
     */
    private Thread permissionExpireThread;

    /**
     * Represents the permissions associated with peerAddress IP installed for this Allocation.
     */
    private final HashMap<TransportAddress, Permission> permissions =
            new HashMap<TransportAddress, Permission>();

    /**
     * The <tt>Thread</tt> which expires the <tt>ChannelBind</tt>s of this <tt>Allocation</tt> and
     * removes them from {@link #channelBindings}.
     */
    ...
    /**
     * Contains the mapping of peerAdress of ChannelBinds to Channelno. This is used to check the
     * peerAddress while creating new Permissions and ChannelBinds.
     */
     ...
     */
    public Allocation(TransportAddress relayAddress, FiveTuple fiveTuple) {
        this(relayAddress, fiveTuple, null, null);
    }

    /**
     * Constructor to instantiate an Allocation without a username and password with the lifetime
     * value.
     * 
     * @param relayAddress the realyAddress associated with this Allocation.
     * @param fiveTuple the fiveTuple associated with this Allocation.
     ...
     */
    public Allocation(TransportAddress relayAddress, FiveTuple fiveTuple, long lifetime) {
        this(relayAddress, fiveTuple, null, null, lifetime);
    }

    /**
     * Constructor to instantiate an Allocation with given relayAddress, fiveTuple, username,
     * passowrd and with default lifetime value.
     * 
     * @param relayAddress the realyAddress associated with this Allocation.
     * @param fiveTuple the fiveTuple associated with this Allocation.
     ...
     */
    public Allocation(TransportAddress relayAddress, FiveTuple fiveTuple, String username,
            String password) {
        this(relayAddress, fiveTuple, username, password, Allocation.DEFAULT_LIFETIME);
    }
    ...
    /**
     * Returns the clientAddress associated with this Allocation. The client address who instianted
     * this allocation.
     */
    ...
    /**
     * Returns the serverAddress associated with this Allocation. The serverAddress on which this
     * allocation request is received.
     */
    ...
    /**
     * Returns the Client Data Connection corresponding to Connection Id for which ConnectionBind
     * Request has been received.
     * 
     * @param connectionId the ConnectionId for which Client Data Connection is to be returned.
    ...
    /**
     * Returns the Peer TCP Data Connection corresponding to Connection Id for which ConnectionBind
     * Request has been received.
     * 
     * @param connectionId the ConnectionId for which Peer TCP Data Connection is to be returned.
    ...
    /**
     * Adds the Connection Id with corresponding Client Data Connection to for which ConnectionBind
     * Request has been received.
     * 
     * @param connectionId the ConnectionId.
     * @param clientDataConn Client Data Connection to corresponding ConnectionId.
     */
    public void addDataConnection(int connectionId, FiveTuple clientDataConn) {
        this.connIdToDataConnMap.put(connectionId, clientDataConn);
    }

    /**
     * Adds the Connection Id corresponding to Peer TCP Data Connection for which ConnectionBind
     * Request has been received.
     * 
     * @param connectionId the ConnectionId.
     * @param peerDataConn Peer TCP Data Connection to corresponding ConnectionId.
     */
     ...
     * 
     * @param connectionId the ConnectionId corresponding to Client Data Connection.
     */
    ...
    /**
     * Returns the lifetime associated with this Allocation. If the allocation is expired it returns
     * 0.
     */
    ...
    /**
     * Sets the time to expire in milli-seconds for this allocation. Max lifetime can be
     * Allocation.MAX_LIFEIME.
     * 
     * @param lifetime the lifetime for this Allocation.
     */
    public void setLifetime(long lifetime) {
        synchronized (this) {
            this.expirationTime =
                    System.currentTimeMillis() + Math.min(lifetime * 1000, Allocation.MAX_LIFETIME);
        }
    }
    ...
    /**
     * Start the Allocation. This launches the countdown to the moment the Allocation would expire.
     */
     ...
     * 
     * @return <tt>true</tt> if this <tt>Allocation</tT> is expired now; otherwise, <tt>false</tt>
     */
    ...
    /**
     * Expires the Allocation. Once this method is called the Allocation is considered terminated.
     */
    public synchronized void expire() {
        expired = true;
        /*
         * TurnStack has a background Thread running with the purpose of removing expired
         * Allocations.
         */
    }

    /**
     * Determines whether this <tt>Allocation</tt> will be expired at a specific point in time.
     * 
     * @param now the time in milliseconds at which the <tt>expired</tt> state of this
     *        <tt>Allocation</tt> is to be returned
     * @return <tt>true</tt> if this <tt>Allocation</tt> will be expired at the specified point in
    ...
    /**
     * Determines whether the ChannelBind request is a BAD request or not. A request is BAD when the
     * same client sends a ChannelBind Request and channel no or peerAddress coincides with existing
     * channel bindings. A request is not bad if the channel no and peerAddress in the ChannelBind
    ...
    public boolean isBadChannelRequest(ChannelBind channelBind) {
        boolean hasChannelNo = this.channelBindings.containsKey(channelBind.getChannelNo());
        boolean hasPeerAddr = this.peerToChannelMap.containsKey(channelBind.getPeerAddress());
        if (hasChannelNo && hasPeerAddr) {
            if (this.channelBindings.get(channelBind.getChannelNo())
                    .equals(channelBind.getPeerAddress())) {
                return false;
            }
    ...
    /**
     * Checks if the Permission is installed for the peerAddress. The port value is ignored.
     * 
     * @param peerAddress the peerAddress for which to check permission.
     * @return true if permission is installed for peerAddress else false.
     */
    public boolean isPermitted(TransportAddress peerAddress) {
        peerAddress = new TransportAddress(peerAddress.getAddress(), 0, peerAddress.getTransport());
     ...
     * 
     * @param channelNo the channel number to check.
     * @return true if the specified channel no. is installed for this allocation.
     */
     ...
     * 
     * @param channelNo the channel no for which to get the peerAddress.
     * @return peerAddress the peerAddress associated with the channelNo in this allocation.
     */
     ...
     * 
     * @return true if no of permissions are less than maximum allowed permissions per Allocation.
     */
     ...
     * 
     * @return true if no of channels are less than maximum allowed channels per Allocation.
     */
    ...
    /**
     * Runs in {@link #channelBindExpireThread} and expires the <tt>ChannelBind</tt>s of this
     * <tt>Allocation</tt> and removes them from {@link #channelBindingings}.
     */
        ...
        try {
            ...
            do {
                synchronized (channelBindings) {
                    ...
                    /*
                     * Is the current Thread still designated to expire the ChannelBinds of this
                     * Allocation?
                     */
                    ...
                    if (channelBindings.isEmpty()) {
                        ...
                        for (Iterator<ChannelBind> i = channelBindings.values().iterator(); i
                                .hasNext();) {
                            ...
                            if (channelBind == null) {
                                ...
                                logger.trace("ChannelBind " + channelBind + " expired");
                                i.remove();
                                this.peerToChannelMap.remove(channelBind.getPeerAddress());
                                channelBind.expire();
                            }
                        }
                    }
            ...
            synchronized (channelBindings) {
                if (channelBindExpireThread == Thread.currentThread())
                    channelBindExpireThread = null;
                /*
                 * If channelBindExpireThread dies unexpectedly and yet it is still necessary,
                 * resurrect it.
                 */
    ...
    /**
     * Runs in {@link #PermissionExpireThread} and expires the <tt>Permission</tt>s of this
        ...
        try {
            ...
            do {
                synchronized (permissions) {
                    ...
                    /*
                     * Is the current Thread still designated to expire the Permissions of this
                     * Allocation?
                     */
                    ...
                    if (permissions.isEmpty()) {
                        ...
                        for (Iterator<Permission> i = permissions.values().iterator(); i
                                .hasNext();) {
                            ...
                            if (permission == null) {
                                ...
                                logger.trace("Permission " + permission + " expired");
                                i.remove();
                                permission.expire();
                            }
                        }
            ...
            synchronized (permissions) {
                if (permissionExpireThread == Thread.currentThread())
                    permissionExpireThread = null;
                /*
                 * If permissionExpireThread dies unexpectedly and yet it is still necessary,
                 * resurrect it.
                 */
    ...
    /**
     * Since an Allocation is uniquely identified by its relay address or five tuple hence we only
     * compare these members.
     */
    @Override
